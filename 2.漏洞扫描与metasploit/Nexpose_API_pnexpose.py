...
class Connection():
    ...
    def list_sites(self):
        ...
        for site in sitesList:
            siteSummaryList.append((site['description'], site['id'], site['name'], site['riskfactor'], site['riskscore']))
    ...
    def site_scan(self, siteid):
        ...
        scanIDStartPosition = response.index('scan-id')+9
        scanIDStopPosition = response.index('"', scanIDStartPosition)
        scanID = response[scanIDStartPosition:scanIDStopPosition]
    ...
    def ad_hoc_report_request(self, call, query, site_ids=[],
        ...
        try: 
            ...
            vulStart = response_data.index('base64')+10
            vulEnd = response_data.index('--', vulStart)
            response = response_data[vulStart:vulEnd]
